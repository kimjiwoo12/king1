...
import com.openai.board.util.HashTagProccess;
...
public class BoardService {
	...
	public String boardWrite(MultipartHttpServletRequest multi,
		...
		try {
			...
			for(int i = 0; i < tList.size(); i++) {
				...
				if(type.equals("NNG") || type.equals("NNP") 
						|| type.equals("NNB")) {
					bDao.tagInsert(td);
					cnt++;
				}
	...
	private void fileUpload(MultipartHttpServletRequest multi, 
		...
		fmap.put("bnum", String.valueOf(b_num));
	...
	public Map<String, List<ReplyDto>> replyInsert(ReplyDto reply){
		...
		try {
			...
			List<ReplyDto> rList = 
					bDao.replySelect(reply.getR_bnum());
	...
	public void fileDownload(BfileDto bfile, 
		...
		try {
			...
			while((length = is.read(buffer)) != -1) {
				//inputstream의 read 메소드는 버퍼의 크기만큼
				//데이터를 읽어와서 읽어온 크기를 알려주는 메소드
